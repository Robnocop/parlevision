...
//
int BlobTracker::getIDBySum(int sum)
{
...
int BlobTracker::getBlinkTime() const
{
    QMutexLocker lock(m_propertyMutex);
    return m_blinkTime;
}

void BlobTracker::setBlinkTime (int num)
{
    QMutexLocker lock(m_propertyMutex);
    if( num >= 0 )
    {
...
int BlobTracker::getThreshold() const
{
    QMutexLocker lock(m_propertyMutex);
    return m_threshold;
}

void BlobTracker::setThreshold(int num)
{
    QMutexLocker lock(m_propertyMutex);
    if( num >= 0 )
    {
...
void BlobTracker::setMinLedSize (int ledsize)
{
    QMutexLocker lock(m_propertyMutex);
...
int BlobTracker::getMinLedSize () const
{
    QMutexLocker lock(m_propertyMutex);
    return m_minLedSize;
}

void BlobTracker::setMaxLedSize (int ledsize)
{
    QMutexLocker lock(m_propertyMutex);
...
int BlobTracker::getMaxLedSize () const
{
    QMutexLocker lock(m_propertyMutex);
    return m_maxLedSize;
}
