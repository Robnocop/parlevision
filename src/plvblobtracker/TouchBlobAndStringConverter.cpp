...
bool TouchBlobAndStringConverter::process()
{
	...
	//assume the closest blob is the one we are interested in, so save the r with ymin
	int ymax = 0;
	...
	{QString blobString = QString("BEGIN_TOUCH#TOUCH_CENTER_X:%1#TOUCH_CENTER_Y:%2#TOUCH_CENTER_Z:%3#").arg(r.x *widthpane  / src.cols).arg(r.y*heightpane/ src.rows).arg(r.z);
		...
		out.append("\n");
		cv::circle(dst2, cv::Point(r.x *widthpane  / src.cols, r.y*heightpane/ src.rows), 20, cv::Scalar(255,0,0), -1, 8,0 ); //draw the center of gravity
	}
