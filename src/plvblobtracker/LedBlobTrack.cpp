...
LedBlobTrack::LedBlobTrack(unsigned int id,
                     Blob& blob,
                     int birthThreshold,
                     int birthWindow,
                     int dieThreshold,
                     ...
                     int trackSize)
{
    d = new LedBlobTrackData(id, blob, birthThreshold, birthWindow, dieThreshold, historySize, trackSize);
...
LedBlobTrack::~LedBlobTrack()
{
    // nothing to do, QSharedDataPointer does ref counting
}

const Blob& LedBlobTrack::getLastMeasurement() const
{
    return d->history.last();
}

/** called when this track did not match any blob */
void LedBlobTrack::notMatched( unsigned int frameNumber )
{
    ++d->age;
    ...
    if( frameNumber - d->history.last().getFrameNr() > d->dieThreshold )
    {
        ...
        d->state = LedBlobTrackDead;
    }
}

void LedBlobTrack::setID(int newid)
{
	d->id = newid;
}

void LedBlobTrack::setBits(bool bit, int measurepoint)
{
...
void LedBlobTrack::addMeasurement( const Blob& blob )
{
	...
	if (d->state == LedBlobTrackDead)
	{

		//qDebug() << "the die threshold is" << (int) d->dieThreshold;
		//threshold was/is set in header to 10
		d->state = LedBlobTrackBirth;
		//qDebug() << "I have come alive";
		//works
	}

    if( d->state == LedBlobTrackBirth )
    {
...
int LedBlobTrack::matches( const Blob& blob ) const
{
    if( d->state == LedBlobTrackDead )
        return 0;
...
void LedBlobTrack::draw( cv::Mat& target ) const
{
